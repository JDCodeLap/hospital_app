# 📒 작업 기록 아카이브 (hospital-app)

> CLAUDE.md가 매 세션 자동 로드되어 너무 길어졌기에, **모든 작업 기록(최신순)**을 이 파일 한 곳에 모아 보관합니다.
> CLAUDE.md에는 작업 일지를 두지 않고 이 파일로의 링크만 둡니다. (이 파일은 자동 로드되지 않음 — 필요할 때만 참고)
> 새 작업을 끝내면 이 안내 바로 아래(맨 위)에 날짜와 함께 추가하세요.

### 2026-06-25 — 🔒 모바일 앱 "재로그인" 정책(콜드스타트·미사용 자동 로그아웃) — 정식 스토리 밖(사용자 요청)
- 계기: 사용자 "모바일에서 앱을 완전 종료/재부팅 후 다시 켜도 자동 로그인되어 있다 — 병원용 앱인데 문제 아니냐". 원인=토큰을 `localStorage`에 저장(앱 꺼도 안 지워짐)+백엔드 토큰 8h. 버그는 아니나 민감정보 앱이라 보안 정책으로 변경. 사용자 결정 = **모바일만 적용(웹 PC는 기존 유지)** + 재로그인 조건 = **완전종료/재부팅 + 일정시간(30분) 미사용**
- 🔑 모바일 식별: 웹/모바일이 같은 배포본이라 구분 필요 → Flutter가 첫 로딩 주소에 표식 `?client=app` 부착(`mobile/lib/main.dart`: `kAppUrl`). 웹은 `isNativeApp()`이 표식 감지 시 `sessionStorage["hospital_client"]="app"` 기록(앱 내 이동으로 쿼리 사라져도 세션 플래그로 유지, 콜드스타트마다 재감지). PC 브라우저는 표식 없어 항상 false → 기존 동작 100% 유지
- 🗄️ 저장소 전환(`frontend/src/lib/auth.ts`): `getStore()`가 모바일이면 **sessionStorage**(앱 프로세스 종료 시 자동 비워짐 → 콜드스타트/재부팅 후 재로그인), 웹이면 기존 **localStorage**. `saveToken/getToken/clearToken` 모두 `getStore()` 사용. `clearToken`은 네이티브 플래그는 유지(로그아웃해도 여전히 앱 안)
- ⏲️ 미사용 타임아웃(모바일 전용): `IDLE_LIMIT_MS`(기본 30분) + `touchActivity()`/`isIdleExpired()`(`hospital_last_active` 기록). 감시 훅 `components/useIdleLogout.ts`(NEW) — 마운트 검사 + 30초 주기 검사 + `pointerdown/keydown` 활동기록(쓰로틀) + `visibilitychange` 복귀 검사, 만료 시 `clearToken`→`/login`. 모든 보호화면 공통 `AppShell.tsx`에서 1회 호출. `saveToken`·`fetchMe` 성공 시 `touchActivity` 갱신
- ✅ 검증: `tsc --noEmit` 0 + `next build` exit 0(라우트 14개 정상) + 신규/수정 파일 ESLint 통과(※ `AppShell.tsx:105`의 set-state-in-effect 경고는 기존 코드라 무관, 빌드는 통과). 웹앱 래핑이라 웹 push→Vercel 자동배포 + 모바일은 표식 들어간 새 APK 재빌드 필요
- ▶▶ 확인 권장: 실기기에서 ①앱 완전종료 후 재실행=로그인화면 ②잠깐 백그라운드 복귀=유지 ③30분 방치=자동 로그아웃(테스트 시 `IDLE_LIMIT_MS` 임시 단축). 남은 것: **🔒 Supabase DB 비밀번호 교체**(여전히 보류)

### 2026-06-23 — ⚡ 체감 속도·UX 개선 세션(서버 리전 이전 + 화면 전환 효과 + ERP UX 요소 6종) — 정식 스토리 밖(사용자 요청)
- 계기: 사용자 "화면 넘어가는 게 부드럽지 않다" → 지연 측정으로 **Railway 리전이 US West**(사용자=한국·DB=서울과 멀어 태평양 왕복)임을 확인. 사용자가 Railway 리전을 **Southeast Asia(Singapore)**로 변경 → 재측정 결과 환자목록 **~985ms → ~570ms(약 42%↓)**, health ~150→~110ms. 무료 호스팅 기준 충분, 추가 단축은 캐싱 검토 여지(보류)
- 🎞️ 화면 전환 부드럽게: `AuthGuard`/`AdminGuard`의 "확인 중…" 텍스트 → **Spinner**(`components/Spinner.tsx`, 초록 회전+라벨)로 교체. `globals.css`에 `page-fade-in` 키프레임(opacity+translateY, GPU 친화, `prefers-reduced-motion` 대응) 추가 → `AppShell` 본문에 `key={pathname}`로 화면 이동마다 fade-in 재생
- 🧩 ERP UX 요소 6종 신설(`components/ui/*`) + 전역 연결:
  - **Toast**(전역 토스트 성공/오류/정보 + `useToast`) — `Providers.tsx`(Toast+Confirm)로 `layout.tsx` 본문 전체 감쌈. 적용: PatientForm 저장, MedicationAlerts 완료, StaffManagement 등록/수정/삭제
  - **Confirm**(약속 기반 확인 대화상자 + `useConfirm`) — StaffManagement 삭제의 `window.confirm` 대체(danger 스타일)
  - **Badge**(상태 색상 뱃지 회/녹/청/황/적) — StaffManagement '비활성'을 빨강 뱃지로
  - **EmptyState**(빈 목록 안내 아이콘+설명+행동버튼) — MedicationAlerts/PatientSearch/PatientFlowBoard(검색·흐름판은 "신규 환자 등록" 유도)
  - **Tooltip**(PC hover 설명, CSS group-hover라 모바일 무해) — 헤더 종 아이콘, StaffManagement 수정/삭제 아이콘 버튼
  - **알림 개수 뱃지**: `AppShell` 종 아이콘 위 빨간 숫자(`/api/medication-alerts` 길이, 60초+화면이동마다 갱신, 9+ 표시)
- ✅ 검증: `tsc --noEmit` 0 + `next build` exit 0(라우트 14개 정상). 프론트 전용(백엔드 무관). 웹앱 래핑이라 Vercel 자동배포 후 앱 껐다 켜면 반영(재설치 불필요) — 사용자 실기기서 스피너·fade 확인 완료
- ▶▶ 남은 것: **🔒 Supabase DB 비밀번호 교체**(여전히 보류, 사용자 "나중에") / 추가 캐싱(선택)

### 2026-06-23 — 🚀 실배포(Railway+Vercel) + 📱 Epic 6 모바일 앱(Flutter WebView APK) 완료 → 🎉 로드맵 전체 완료
- **배포**: 서버=**Railway**(`hospitalapp-production-d367.up.railway.app`, Root Directory=backend, railway.json/Nixpacks, 환경변수 DATABASE_URL·JWT_SECRET_KEY·CORS_ORIGINS) / 화면=**Vercel**(`hospital-app-xi-ruby.vercel.app`, Root=frontend, NEXT_PUBLIC_API_BASE_URL) / DB=Supabase(서울). CORS_ORIGINS에 localhost+vercel 둘 다. 헬스체크·로그인·CORS 헤더 전수 확인 완료. 가이드=`docs/배포가이드.md`
  - ⚠️ 삽질 메모: Railway 변수에 `.env`의 `=`까지 붙여넣어 값이 `=postgresql...`/`=0c9b...`로 들어가 **헬스체크 실패** → 맨 앞 `=` 제거하니 해결. Render→Railway 전환(무료 잠듦 회피, 사용자 선택)
- **Epic 6 (Story 6.1)**: `mobile/`에 Flutter 앱 신설. webview_flutter로 배포 웹주소(kWebUrl) 감쌈 — `lib/main.dart`(WebView+로딩스피너+뒤로가기=goBack/종료), AndroidManifest INTERNET 권한+`android:label="병원 안전관리"`, flutter_launcher_icons로 전용 아이콘(초록#16A34A 배경+흰 십자, adaptive 포함) 생성. `flutter build apk --release` → `app-release.apk`(41MB). 바탕화면에 `병원안전관리.apk`/`병원안전관리.zip`(카톡 배포용, 19MB)
  - 도구 환경 신규 구축: Flutter 3.44.3(C:\dev\flutter, PATH등록) + Android Studio(SDK 36) + cmdline-tools(수동 설치) + 라이선스 동의. 빌드는 디버그키 서명(사이드로드용 충분, 스토어용 키스토어는 추후)
- **배포 특성**: 웹앱 래핑이라 웹 push→Vercel 자동배포 시 앱도 자동 최신(주소 바뀔 때만 재빌드). 카톡은 .apk 직접전송 차단→링크/zip로 배포
- ▶▶ 남은 것: **🔒 Supabase DB 비밀번호 교체**(배포 중 캡처·카톡에 노출됨 — 진행 예정) / 스토어용 키스토어 서명(선택) / iOS(선택)

### 2026-06-23 — 📱 모바일 하단 탭바 정리 + 햄버거(☰) 서랍 메뉴 — 정식 스토리 밖(UX 개선)
- 계기: 폰 하단 탭바가 5~6개(일반 5·관리자 6)로 모바일 권장(3~5)을 넘어 비좁음. 사용자 선택 = "방법 4(상단 햄버거 서랍으로 보조 메뉴 이동)"
- `AppShell.tsx`: `NAV`를 **`CORE_NAV`(대시보드·환자·흐름판·알림 4개)** + **`SECONDARY_NAV`(스키마)** + `ADMIN_NAV`로 분리. **폰 하단 탭바는 CORE 4개만**, 보조(스키마·관리자)는 상단 **☰ 서랍(drawer)**에 모음. 서랍=전체 메뉴(핵심+보조+관리자), 좌측 슬라이드+배경오버레이+Esc 닫기+body 스크롤잠금+주소변경/항목클릭 시 자동 닫힘. **PC 사이드 메뉴는 `fullNav` 전체 펼침(기존 유지)**. 상단바 좌측에 ☰ 버튼(md:hidden) 추가
- ✅ 검증: `tsc --noEmit` 0. 프론트 전용 변경(백엔드 재시작 불필요, Next 자동반영). 실기기/미리보기 클릭 검증은 다음
- ▶▶ 다음 후보: 모바일 미리보기로 ☰·하단바 확인 / 환자 등록·수정·중복 클릭 검증 / **Epic 6(모바일 패키징)**

### 2026-06-23 — ✍️ 환자 기본정보 등록·수정 기능 추가 — 정식 스토리 밖(사용자 요청, correct-course 생략)
- 계기: 사용자 지적 "환자 기록을 수정할 방법이 없는듯?" → 실제로 환자 **기본정보(이름·생년월일·성별·전화·주민번호·알레르기·등록번호)는 등록도 수정도 불가**(앱이 조회 전용으로 설계됨, Epic 1~6 어디에도 환자 CRUD 없음). 기능이 작고 명확해 `bmad-correct-course` 정식 절차는 생략하고 이 한 줄 기록으로 갈음(사용자 선택)
- 🗄️ 백엔드(`main.py`): `POST /api/patients`(신규 등록 — 이름 필수 422, 등록번호 중복 409·비우면 `generate_registration_number`로 자동 부여 P####, 등록 시 `StageEntry` '접수' 진입시각 기록해 4.4 대기시계 시작) + `PATCH /api/patients/{id}`(부분 수정 — 보낸 필드만, 등록번호 변경 시 중복검사, **broadcast로 상세화면 실시간 갱신**). 둘 다 `get_current_user`(로그인 직원이면 누구나 — 기본정보는 5.3 영역게이트 대상 아님, 항상 표시되던 것과 일관)
- ★ 보안 포인트: 수정 시 **주민번호는 새 값 입력 시에만 교체**(빈 값=기존 유지) — 조회는 마스킹값(`901020-1******`)만 내보내므로, 마스킹값으로 원본 덮어쓰기 방지(StaffUpdate의 password와 동일 철학). 응답도 항상 `mask_resident_id`
- 🖥️ 프론트: `PatientForm.tsx`(NEW: 등록·수정 공용 — 수정 모드는 GET 프리필+주민번호칸만 비움, 401/403 로그아웃·409/422 detail 표시·중복전송 락) + `PatientEditForm.tsx`(NEW: useParams로 id 읽는 연결부품, 상세화면과 동일 패턴) + `app/patients/new/page.tsx`·`app/patients/[id]/edit/page.tsx`(NEW, AuthGuard 보호) + 검색화면 "**+ 신규 환자 등록**" 버튼 + 상세헤더 "**정보 수정**" 버튼
- 🔁 보강(사용자 요청): 등록번호 중복 시 단순 문구 대신 **그 번호를 쓰는 환자 정보를 보여주며 "변경 불가" 안내**. 백엔드 409 `detail`을 객체(`message` + `conflict_patient`{id·name·reg·age·gender})로 싣는 `registration_conflict_error` 헬퍼 신설(POST·PATCH 사전검사+IntegrityError 폴백 4곳 공통 사용, PATCH는 `attempted_reg`로 겹친 번호 추적) + `PatientForm`이 그 환자 카드(아바타·이름·번호·"환자 보기" 링크) 표시·번호 수정 시 안내 자동 해제. 처방 충돌 409와 같은 객체-detail 패턴
- ✅ 검증: 백엔드 `py_compile` OK / 프론트 `tsc --noEmit` 타입오류 0. (실서버 수동 클릭 테스트는 미실시 — 다음 세션 권장)
- ▶▶ 다음 후보: 실서버에서 등록·수정·중복 클릭 검증 / 진단·검사·수납 편집(미구현 영역) / 커밋 / **Epic 6(모바일)**

### 2026-06-23 — 🎨 UI/UX 개선 세션 (서울 DB·낙관적 업데이트·다크 테마·환자정보·단계색) — 정식 스토리 밖
- 🌏 **Supabase 도쿄→서울 전환**: `.env` DATABASE_URL을 서울 리전(`aws-1-ap-northeast-2.pooler.supabase.com`, ref `hrxzzkebkusfybnmgkqt`, Session pooler)으로 교체 → 응답 속도 개선. 테이블·시드 자동 생성 확인. **기존 도쿄 프로젝트(ref `lqzakafihkfoenimftno`)는 사용자가 삭제 예정**(보존 데이터 없음). DB 비번은 `.env`에만(문서 비기재)
- ⚡ **낙관적 업데이트**(원격 DB 왕복 대기 제거): 필수 절차 체크리스트 토글 + 인계메모(작성·수정·삭제)를 탭 즉시 화면 반영 + 실패 시 롤백 + 서버 재조회 오면 오버레이 정리. React 권장 패턴(렌더 중 prev 비교)으로 prop 변화 리셋. 커밋 `d7b4648`. ★입력 반응성 전수 조사 결과: 처방삭제·직원삭제는 낙관적 미적용(서버 거부 가능 케이스라 미룸), 처방 추가/수정은 알레르기 안전검사라 현 방식 유지, safety-ack·투약완료·기준값저장은 이미 즉시형
- 🎨 **Supabase 다크 테마**(`globals.css`, tweakcn supabase.json 기반): 화면 컴포넌트가 쓰는 커스텀 토큰(`--color-*`)을 다크 팔레트로 교체 → `.dark` 토글 없이 `:root` 자체가 다크(검정 배경+초록 포인트). 글꼴은 한글 위해 Pretendard 유지(Outfit 한글 미지원). 하드코딩 색은 `bg-black/60`(모달 오버레이) 1개뿐이라 깔끔. 커밋 `7a982d3`
- 🧑‍⚕️ **환자 정보 보강 + 단계별 색상**: Patient에 `phone`·`resident_id` 컬럼(멱등 ALTER + 생년월일·성별 기반 백필) + `mask_resident_id`(응답 `901020-1******` 마스킹, **원본 미노출** — 주민번호는 민감정보, DB 더미값·실운영 시 암호화/접근통제 필요) + 상세 헤더에 전화·주민 표시. 단계색 공통 `lib/stage.ts`(접수=회색·진료=파랑·검사=주황·수납=초록)을 검색목록·흐름판·상세헤더·타임라인에 적용. 커밋 `41a0262`
- 🛠️ **`/commit` 글로벌 커스텀 명령어** 생성(`~/.claude/commands/commit.md`) — 모든 프로젝트에서 한국어 Conventional Commits 커밋 도우미
- ⚠️ **상태**: 로컬 커밋 5개(`perf d7b4648`·`style 7a982d3`·`feat 41a0262` 등)가 **origin보다 앞섬 — push 필요**. 서버: 백엔드 8000(서울 DB)·프론트 3000 실행 중. `.env`는 git 미추적
- ▶▶ 다음 후보: 로컬 커밋 push / 일본 DB 삭제(사용자) / 처방·직원 삭제 낙관적 / **Epic 6(모바일)** / Epic 5 회고

### 2026-06-23 — 🔍 Story 5.5 코드 리뷰 완료 → done! ⚙️ 🎉 Epic 5 전체 완료! (기준값 설정, FR14)
- 검사관 3종 병렬(Blind·Edge·Acceptance) — **Acceptance Auditor AC 1~8 전부 PASS**(스코프 크리프·미구현·모순 0, "승인 가능"). Blind·Edge가 공통 지목한 1건만 patch.
- **patch 1건 적용**: `get_overdue_threshold` **읽기측 범위 클램프**(`max(MIN, min(MAX, value))`) — PUT 검증(1~1440)과 일치시켜, 범위 밖 저장값(0·음수·거대값)이 DB에 들어가도 전원 대기초과로 깨지지 않게 방어. 현재 쓰기 경로(PUT·시드)엔 유입 없으나 저비용 견고성. 클램프 확인(0→1·-5→1·99999→1440·30→30)·8000 재기동 OK
- defer 3건 → `deferred-work.md`: ①PUT 업서트 동시성+시드 레이스 PK 충돌(단일워커 무해) ②저장 fetch AbortController 부재(로드와 비대칭·React 19 무해) ③naive datetime tz(앱 전반 기존 추적)
- dismiss 8건: bool→1(number input·범위내 무해)·Number 1e3(상한이 막음)·저장응답이 입력칸 덮어씀·settings=null 무반응(도달 불가)·형식오류 메시지·422 detail 배열(fallback 안전)·문구 차이(이중방어 의도)·PUT 에코(정상)
- 상태: **5-5 = done** → 🎉 **Epic 5(관리자 페이지) 전체 완료**(5.1~5.5 done). sprint-status의 `epic-5: done` 갱신
- ▶▶ **다음: Epic 6 (Flutter WebView 모바일 앱 패키징)** = 마지막 에픽. 또는 Epic 5 회고(`bmad-retrospective`)

### 2026-06-23 — 🛠️ Story 5.5 개발 완료! (기준값 설정) → review ⚙️ (FR14, Epic 5 마지막)
- 🗄️ 백엔드: `AppSetting`(key/value) 테이블 신설(create_all, 마이그레이션 0) + `get_overdue_threshold` 헬퍼(설정값→없거나 깨지면 30 폴백) + `stage_wait_info`에 `threshold` 파라미터 주입(전역 가변 금지·하위호환) + 호출부 2곳(`list_patients` 4.4·`admin_dashboard` 5.4) 요청당 1회 주입 + `GET`·`PUT /api/admin/settings`(get_current_admin 보호, 1~1440 검증·업서트) + lifespan 멱등 시드. `STAGE_OVERDUE_MINUTES=30`은 기본값/폴백으로 유지(삭제 안 함=무회귀)
- 🖥️ 프론트: `AdminSettings.tsx`(NEW: 현재값 로드+숫자입력+저장, busyRef·401/403 로그아웃·422 detail 표시·클라 범위 이중검증) + `admin/settings/page.tsx`(NEW) + `AdminOverview`(기준값 카드 활성화, `COMING_SOON` 빈 배열+준비중 섹션 조건부 렌더)
- ★ 핵심: 상수는 기본값으로 잔존 / 파라미터 주입(전역 가변 X) / "읽을 때 계산"이라 저장만 하면 4.4·5.4 자동 반영(캐시 무효화 불필요) / 진짜 자물쇠=백엔드(403 게이트 + 422 검증)
- ✅ 검증: 백엔드 실호출 **19/19 PASS**(8001 별도 포트) — GET/PUT·**반영**(dashboard threshold=10, patients is_overdue 10분 기준 재계산 불일치 0)·0/-5/1441/abc/30.5 전부 422·nurse 403·미인증 401·경계 1/1440·원복 30 / 프론트 lint(0 errors)·build(`/admin/settings` 정적) PASS. 8000 새 코드 재기동 완료
- **다음 할 일:** 🔍 `bmad-code-review` → 통과 시 **Epic 5(관리자 페이지) 전체 완료** 🎉 (남은 건 Epic 6 모바일 패키징)

### 2026-06-23 — 🖥️ 새 PC에 개발환경 재구성 + 실서버 기동 + 🔐 DB 비번 노출 정리
- 계기: GitHub `JDCodeLap/hospital_app`(**public**) 새로 클론. 이 PC엔 .venv·node_modules·.env 모두 없었음(클론본이라 당연)
- 환경 재설치: **Python 3.14.6**(이 PC 기본, 이전 3.12 아님 — cp314 휠 전부 존재해 `requirements.txt` 8종 정상 설치) + `backend/.venv` 생성 + 프론트 `npm install`(취약점 2건은 흔한 경고, 무시)
- `backend/.env` 재작성(gitignore됨): `DATABASE_URL`(Supabase **Session pooler**, `postgres.lqzakafihkfoenimftno`@`aws-1-ap-northeast-1.pooler.supabase.com:5432`) + 새 랜덤 `JWT_SECRET_KEY` + CORS. ★연결 테스트로 pooler·비번 조합 확인 후 작성(psycopg 키워드 인자 연결=URL 인코딩 회피)
- 🔐 **보안 조치(사용자 지적 "작업기록에 DB 비번 적혀도 되나?")**: CLAUDE.md·작업기록-아카이브.md에 **DB 비번이 평문 노출**돼 있었고 레포가 public이라 인터넷 공개 상태였음 → ①사용자가 **Supabase에서 DB 비번 재설정** ②두 문서에서 비번 글자 **제거**(이후 .env에만 보관). ★단 **과거 커밋엔 옛 비번이 남으므로 "재설정"이 진짜 해결책**(문서 수정만으론 불충분)이라 비번 교체가 핵심
- ⚠️ 데모 계정 비번(시드 `main.py` + 다수 BMAD 문서에 분산)은 **로컬 데모용이라 보류** → 실배포 시 일괄 변경(이미 `deferred-work.md`에 과제화)
- ✅ 실서버 검증: 백엔드 8000 `/api/health` ok + **nurse1 로그인→토큰(119자)→`/api/auth/me`(role=staff) 성공**(Supabase 테이블 생성·시드 자동 완료) / 프론트 3000 `next dev`(Next.js 16.2.9 Turbopack) HTTP 200. ★실제 비밀번호·시크릿은 전부 `backend/.env`에만(문서 비기재)
- ⚠️ 서버 2개 이 세션 백그라운드 실행 중(창 닫으면 종료). 문서 정리는 로컬 수정 상태(커밋 미실행 — 사용자 요청 시)
- ▶▶ 환경 준비 완료 → 이어서 **Story 5.5 (기준값 설정, FR14) = Epic 5 마지막** 진행 가능
...
- `backend/.env` 작성(gitignore됨): `DATABASE_URL`(Supabase **Session pooler, IPv4, aws-1-ap-northeast-1**) + 랜덤 `JWT_SECRET_KEY` + CORS
  ...
  - ★ 비번 혼선: 처음 입력한 비번이 틀려 재설정 후 연결 성공 (★ 실제 DB 비밀번호는 `backend/.env`에만 보관 — 보안상 문서에 적지 않음)
- 백엔드 8000·프론트 3000 기동 성공: `/api/health` ok, nurse1 로그인 토큰 발급, Supabase에 테이블 생성+시드 자동 완료. **이제 백엔드 런타임 검증 가능**(그동안 5.2·5.3은 코드검토만)
- 로그인 계정: nurse1 / admin1 (직원 / 관리자). 비밀번호와 DB 비번은 `backend/.env` 및 시드 코드에만 보관 — 문서에 비기재(보안)
